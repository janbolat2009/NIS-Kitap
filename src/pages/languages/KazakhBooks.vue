...
>
  ...
  <p><strong>Авторы:</strong> {{ book.author }}</p>
  <p><strong>Жанры:</strong> {{ book.genre }}</p>
  <p><strong>Жылы:</strong> {{ book.year }}</p>
  <p><strong>Көшірме саны:</strong> {{ book.copies }}</p>
</div>
         <p v-if="filteredBooks.length === 0" class="no-books">
           Таңдалған жанрға арналған ағылшын кітаптары табылмады немесе деректер жүктелуде.
         </p>
       </div>
     </div>
   </template>
...
export default {
  ...
  computed: {
    uniqueGenres() {
      ...
      const genres = new Set(this.books.filter(book => book.language === 'Қазақ').map(book => book.genre));
      return Array.from(genres).sort();
    },
    filteredBooks() {
      if (!Array.isArray(this.books)) return [];
      let result = this.books.filter(book => book.language === 'Қазақ');
  ...
  methods: {
    goToBookDetail(book) {
      const title = encodeURIComponent(book.title); // Кодируем title для URL
